...
def detect_and_correct_rotated_regions(image: np.ndarray) -> np.ndarray:
    """
    Detect text regions with significant local rotation and correct them.
    Handles date stamps that are tilted but not circular.
    """
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    _, binary = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)

    # find contours of text-like regions
    ...
    for contour in contours:
        ...
        # only process medium-sized regions (likely text blocks, not noise or full page)
        ...
        # only correct if significantly rotated (more than 5 degrees)
        if abs(angle) > 5:
...
def detect_circular_stamps(image: np.ndarray) -> list[tuple]:
    """
    Detect circular stamp regions. Returns list of (x, y, radius).
    These regions contain rotated text that can't be reliably OCR'd.
    """
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
